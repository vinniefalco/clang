...
// expected-error@+1 {{only applies to global variables}}
void test_func_local(ATTR int param) {
  ATTR int x = 42;                     // expected-error {{only applies to}}
  // expected-error@+1 {{'constinit' attribute cannot be applied to a non-defining declaration of an 'extern' variable}}
  ATTR extern int y;
}
...
ATTR extern "C" E c;       // expected-error {{non-defining declaration of an extern "C" variable}}
...
void test_basic_start_static_2_1() {
  ...
  ATTR static const int &local_init = non_global;
  // expected-error@-1 {{variable does not have a constant initializer}}
  // expected-note@-2 {{required by 'constinit' attribute here}}
#if __cplusplus >= 201103L
  // expected-note@-4 {{reference to 'non_global' is not a constant expression}}
...
// expected-note@-1 {{required by 'constinit' attribute here}}
#if __cplusplus >= 201103L
...
struct TT1 {
  ...
  ATTR static thread_local const int &tl_temp_init; // expected-note {{required by 'constinit' attribute here}}
#endif
};
...
struct TT2 {
  ...
  ATTR static PODType pod_copy_init; // expected-note {{required by 'constinit' attribute here}}
#if __cplusplus >= 201402L
  ATTR static constexpr LitType lit = {};
  ATTR static const NonLit non_lit;
  ATTR static const NonLit non_lit_list_init;
  ATTR static const NonLit non_lit_copy_init; // expected-note {{required by 'constinit' attribute here}}
#endif
};
...
// Test non-trivial non-constexpr constructors.
namespace test_non_trivial_ctor {
struct Foo {
  int x = 1;
};
ATTR Foo f; // OK ?
...
// expected-note@-4 {{required by 'constinit' attribute here}}
#endif
