...
void LookupOverloadedBinOp(Sema &S, OverloadCandidateSet &CandidateSet,
                           SourceLocation OpLoc, BinaryOperatorKind Opc,
                           const UnresolvedSetImpl &OrigFuncs,
                           const UnresolvedSetImpl &ThreeWayFuncs, Expr *LHS,
                           Expr *RHS, bool PerformADL,
                           bool AllowRewrittenCandidates) {
  Expr *Args[2] = {LHS, RHS};
  LHS = RHS = nullptr;

  assert(!Args[0]->isTypeDependent() || !Args[1]->isTypeDependent());
  assert(Opc != BO_PtrMemD);
  assert(Opc != BO_Assign || Args[0]->getType()->isOverloadableType());
  ...
  S.AddFunctionCandidates(OrigFuncs, Args, CandidateSet);

  // Add operator candidates that are member functions.
  S.AddMemberOperatorCandidates(Op, OpLoc, Args, CandidateSet);
  ...
  if (Opc != BO_Assign && PerformADL)
    S.AddArgumentDependentLookupCandidates(OpName, OpLoc, Args,
                                           /*ExplicitTemplateArgs*/ nullptr,
                                           CandidateSet);

  // Add builtin operator candidates.
  S.AddBuiltinOperatorCandidates(Op, OpLoc, Args, CandidateSet);

  // C++2a Add rewritten and synthesized operator candidates.
  if (S.getLangOpts().CPlusPlus2a && AllowRewrittenCandidates &&
      BinaryOperator::isComparisonOp(Opc))
    S.AddRewrittenOperatorCandidates(Op, OpLoc, Args, ThreeWayFuncs, OrigFuncs,
                                     CandidateSet, PerformADL);
}
...
/// is well formed.
void Sema::AddRewrittenOperatorCandidates(
    OverloadedOperatorKind Op, SourceLocation OpLoc, ArrayRef<Expr *> InputArgs,
  ...
  auto AddCandidates = [&](ArrayRef<Expr *> Args,
    ...
    AddFunctionCandidates(ThreeWayFns, Args, CandidateSet);
    ...
    for (auto It = std::next(CandidateSet.begin(), InitialSize);
      ...
      assert(Ovl.getRewrittenKind() == Kind);
      ...
      if ((Ovl.RewrittenInfo = CandidateSet.lookupRewrittenCandidateInfo(
        ...
        UnresolvedSet<1> ThreeWayFuncsDummy;
...
RewrittenOverloadCandidateInfo::RewrittenOverloadCandidateInfo(
  ...
  NewCand->Conversions = Candidates.allocateConversionSequences(NumConversions);
  for (unsigned I = 0; I < NumConversions; ++I)
    NewCand->Conversions[I] = Rewritten.Conversions[I];
  if (!Rewritten.Viable && Rewritten.FailureKind == ovl_fail_bad_deduction) {
...
enum OverloadCandidateKind {
  ...
  oc_reversed_rewritten_comparison_operator
};
...
ClassifyOverloadCandidate(Sema &S, NamedDecl *Found, FunctionDecl *Fn,
  ...
  OverloadCandidateKind Kind = [&]() {
    ...
    if (ROC == ROC_AsThreeWay)
      return oc_rewritten_comparison_operator;
    if (ROC == ROC_AsReversedThreeWay)
      return oc_reversed_rewritten_comparison_operator;
...
static void DiagnoseBadConversion(Sema &S, OverloadCandidate *Cand,
  ...
  std::pair<OverloadCandidateKind, OverloadCandidateSelect> FnKindPair =
      ClassifyOverloadCandidate(S, Cand->FoundDecl, Fn, FnDesc, Cand->getRewrittenKind());
...
ExprResult Sema::CreateOverloadedBinOp(SourceLocation OpLoc,
  ...
  OverloadCandidateSet CandidateSet(OpLoc, OverloadCandidateSet::CSK_Operator);
  LookupOverloadedBinOp(*this, CandidateSet, OpLoc, Opc, OrigFuncs,
                        ThreeWayFuncs, Args[0], Args[1], PerformADL,
  ...
  switch (CandidateSet.BestViableFunction(*this, OpLoc, Best)) {
  case OR_Success: {
    const OverloadCandidate &Ovl = *Best;
    Expr *NewArgs[2] = {Args[0], Args[1]};
    if (Ovl.getRewrittenKind() == ROC_AsReversedThreeWay)
      std::swap(NewArgs[0], NewArgs[1]);
    BinaryOperatorKind RewrittenOpc = Ovl.getRewrittenKind() ? BO_Cmp : Opc;

    ExprResult Res =
        BuildBinaryOperatorCandidate(OpLoc, RewrittenOpc, Ovl, NewArgs[0],
                                     NewArgs[1], HadMultipleCandidates);
    if (Res.isInvalid())
      return ExprError();
    ...
    if (Opc != BO_Cmp) {
      // Build the rewritten expression using the three-way comparison
      // expression as an operand. Now attempt to build the full expression
      // '(LHS <=> RHS) @ 0' using the evaluated operand and the literal 0.
      llvm::APInt I =
          llvm::APInt::getNullValue(Context.getIntWidth(Context.IntTy));
      Expr *Zero =
          IntegerLiteral::Create(Context, I, Context.IntTy, SourceLocation());
      ...
      Res = BuildBinaryOperatorCandidate(
          OpLoc, Opc, *RewrittenOvl, NewLHS, NewRHS,
          Ovl.RewrittenInfo->HadMultipleCandidates);
