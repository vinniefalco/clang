...
void Parser::ParseCXXContractAttributeSpecifier(ParsedAttributes &attrs,
  ...
  if (IsValid) {
    ...
    if (OptName) {
      assert(getCurScope());
      assert(DC);
      llvm::outs() << OptName->getName() << '\n';
      auto &Ctx = Actions.getASTContext();
      QualType Ty = Ctx.DependentTy;
      OptD = VarDecl::Create(Ctx, DC, OptIdentLoc, OptIdentLoc, OptName, Ty,
                             Ctx.getTrivialTypeSourceInfo(Ty, OptIdentLoc),
                             SC_None);
      OptD->setImplicit(false);
    }
    if (OptD) {
